...
def create_runfiles(pdb, chain): #This function creates a runfile used for performing a Position Scan in FoldX
	...
	for i in xrange(0, len(residue_lists)):
		filename = "../tacc_files/runfiles/runfile_" + pdb + "_" + chain + "_" + str(i+1) + ".txt"
...
def create_paramlist(pdb, chain, runfiles):	
	i = 1
	out = open("../tacc_files/paramlist_mutate_pdbs", "a+") 
...
def main(argv = sys.argv):
	...
	subprocess.call("mkdir -p ../tacc_files/pdb_lists", shell = True) #Open directories for the files
	subprocess.call("mkdir -p ../tacc_files/runfiles", shell = True)
	for i in xrange(len(pdbs)):	 #For each pdb in the dataset
		filename = "../tacc_files/pdb_lists/pdb_list_" + pdbs[i] + "_" + chains[i] + ".txt"
		out = open(filename, "w")
