...
def create_residue_lists(pdb, chain):
	...
	try:
		...
		s=p.get_structure('X', "../repaired_pdbs/RepairPDB_" + pdb + "_" + chain + ".pdb")
...
def create_runfiles(pdb, chain): #This function creates a runfile used for performing a Position Scan in FoldX
	...
	for i in xrange(0, len(residue_lists)):
		filename = "../foldx_runfiles/runfiles/runfile_" + pdb + "_" + chain + "_" + str(i+1) + ".txt"
...
#This function creates a paramlist used mutating the using launcher on a HPC cluster (We used TACC, which is SGE)
def create_paramlist(pdb, chain, runfiles):	
	i = 1
	out = open("../foldx_runfiles/paramlist_mutate_pdbs", "a+") 
...
def main(argv = sys.argv):
	...
	subprocess.call("mkdir -p ../foldx_runfiles/pdb_lists", shell = True) #Open directories for the files
	subprocess.call("mkdir -p ../foldx_runfiles/runfiles", shell = True)
	for i in xrange(len(pdbs)):	 #For each pdb in the dataset
		filename = "../foldx_runfiles/pdb_lists/pdb_list_" + pdbs[i] + "_" + chains[i] + ".txt"
		out = open(filename, "w")
